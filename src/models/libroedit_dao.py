...
def _pk(table: str) -> str:
    try:
        column = first_existing_column(table, ["ID_EDIT_LIB", "ID_LIBRO_EDIT", "ID_EDIT", "ID"])
    except RuntimeError as exc:
        raise RuntimeError(f"No se identificó PK en {table}.") from exc
    if get_col_datatype(table, column) != "NUMBER":
        raise RuntimeError(f"La PK detectada en {table} debe ser numérica, se obtuvo {column}.")
    return column
...
def _parent_pk() -> Optional[str]:
    ...
    if not table:
        return None
    try:
        column = first_existing_column(table, ["ID_LIBRO_EDIT", "ID_EDIT_LIB", "ID_EDIT", "ID"])
    except RuntimeError as exc:
        raise RuntimeError("LIBRO_EDIT requiere una PK numérica.") from exc
    if get_col_datatype(table, column) != "NUMBER":
        raise RuntimeError("La PK de LIBRO_EDIT debe ser numérica.")
    return column
...
def _ensure_parent(libro_id: int, editorial_id: int, fecha_iso: Optional[str]) -> int:
    ...
    if pk and not table_has_identity(table, pk):
        sequence = find_sequence_for(table, pk)
        if sequence:
            rid_row = query_one(f"SELECT {sequence}.NEXTVAL AS ID FROM DUAL")
            if not rid_row or rid_row.get("ID") is None:
                raise RuntimeError(f"No se pudo obtener NEXTVAL de {sequence}")
            payload["id"] = int(rid_row["ID"])
            columns.append(pk)
            values.append(":id")
        else:
