...
def listar() -> List[Dict[str, object]]:
    sql = """
        SELECT ID_LIBRO, TITULO, SUBTITULO, ISBN, ANO_PUBL, NUM_COPIAS, NUM_PAGINAS,
               FECHA_REGISTRO, DESCRIPCION, CLASIFICACION, PERTENECE_GRUPO,
               ESTADO_FISICO, ID_VAREDIT, ID_GENERO, ID_IDIOMA
        FROM LIBRO
...
def crear(data: Dict[str, object]) -> int:
    payload = {**data}
    payload.setdefault("ID_LIBRO", _next_id())
    sql = """
    INSERT INTO LIBRO (
      ID_LIBRO, TITULO, SUBTITULO, ISBN, ANO_PUBL, NUM_COPIAS, NUM_PAGINAS,
      FECHA_REGISTRO, DESCRIPCION, CLASIFICACION, PERTENECE_GRUPO,
      ESTADO_FISICO, ID_VAREDIT, ID_GENERO, ID_IDIOMA
    ) VALUES (
      :ID_LIBRO, :TITULO, :SUBTITULO, :ISBN, :ANO_PUBL, :NUM_COPIAS, :NUM_PAGINAS,
