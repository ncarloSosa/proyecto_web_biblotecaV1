...
def reporte() -> List[Dict[str, object]]:
    sql = (
        "SELECT ID_LIBRO, TITULO, ISBN, NUM_COPIAS, NUM_PAGINAS, ESTADO_FISICO, "
        "CLASIFICACION, FECHA_REGISTRO, ID_VAREDIT, ID_GENERO, ID_IDIOMA "
        "FROM LIBRO ORDER BY FECHA_REGISTRO DESC"
    )
    return query_all(sql)
