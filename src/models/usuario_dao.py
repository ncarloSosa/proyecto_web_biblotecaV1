...
def listar() -> List[Dict[str, object]]:
    return query_all("SELECT * FROM USUARIO ORDER BY ID_USUARIO DESC")


def obtener(id_usuario: int) -> Optional[Dict[str, object]]:
    return query_one("SELECT * FROM USUARIO WHERE ID_USUARIO = :id", {"id": id_usuario})


def buscar_por_nombre(nombre: str) -> Optional[Dict[str, object]]:
    return query_one("SELECT * FROM USUARIO WHERE UPPER(NOMBRE) = UPPER(:nombre)", {"nombre": nombre})
