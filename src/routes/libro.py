...
def _build_data(form) -> Dict[str, object]:
    ...
    def _first_value(names: List[str]) -> str | None:
        for name in names:
            if not name:
                continue
            value = form.get(name)
    ...
    return {
        ...
        "EDITORIAL_ID": _parse_int(
            _first_value(["EDITORIAL_ID", "ID_VAREDIT", "NUM_EDITORIAL", "ID_EDITORIAL"])
        ),
        "ID_GENERO": _parse_int(_first_value(["ID_GENERO", "GENERO_ID"])),
        "ID_IDIOMA": _parse_int(_first_value(["ID_IDIOMA", "IDIOMA_ID"])),
    }
