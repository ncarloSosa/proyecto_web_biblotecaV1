...
from src.models import editorial_dao, genero_dao, idioma_dao, libro_dao, prestamo_dao
...
def _build_data(form) -> Dict[str, object]:
    ...
    ano_publicacion = _parse_date(form.get("ANO_PUBLICACION"), allow_year=True)
    ...
    return {
        ...
        "PRESTAMO_ID_PRESTAMO": _optional_fk("PRESTAMO_ID_PRESTAMO"),
        "IDIOMA_ID_IDIOMA": _optional_fk("IDIOMA_ID_IDIOMA"),
    }
...
def reporte_csv():
    ...
    for row in data:
        writer.writerow([row.get(col) or "" for col in headers])
