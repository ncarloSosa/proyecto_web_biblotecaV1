...
def _build_data(form) -> Dict[str, object]:
    ...
    fecha = _parse_date(form.get("FECHA_EVENTO"))
    usuario_id = _parse_int(
        _first_value(form, ["USUARIO_ID", "USUARIO_ID_USUARIO", "ID_USUARIO", "usuario_id"]),
        "un usuario",
    )
    libro_id = _parse_int(
        _first_value(form, ["LIBRO_ID", "LIBRO_ID_LIBRO", "ID_LIBRO", "libro_id"]),
        "un libro",
    )
    return {
        "ACCION": accion,
