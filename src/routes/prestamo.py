...
def _build_data(form) -> Dict[str, object]:
    fecha_prestado = _parse_date(form.get("FECHA_PRESTADO"))
    if not fecha_prestado:
        raise ValueError("La fecha de préstamo es obligatoria.")
    fech_caduc = _parse_date(form.get("FECH_CADUC"))
    estado = _trim(form.get("ESTADO"), 20)
    estado_fisic = _trim(form.get("ESTADO_FISIC"), 20)
    if not estado or not estado_fisic:
        raise ValueError("Los estados son obligatorios.")
    id_libro = _parse_int(form.get("ID_LIBRO"))
    id_usuario = _parse_int(form.get("ID_USUARIO"))
    if id_libro is None or id_usuario is None:
        raise ValueError("Debes seleccionar libro y usuario.")
    return {
        "FECHA_PRESTADO": fecha_prestado,
        "FECH_CADUC": fech_caduc,
        "ESTADO": estado,
        "ESTADO_FISIC": estado_fisic,
        "ID_LIBRO": id_libro,
